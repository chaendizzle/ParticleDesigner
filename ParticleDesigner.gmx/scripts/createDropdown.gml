///createDropdown(X, Y, W, H, options, font, textcol, fillcol, outlinecol, textalpha, fillalpha, outlinealpha)
var menu = instance_create(0, 0, obj_dropdown);
menu.RelX = argument0;
menu.RelY = argument1;
menu.X = menu.RelX;
menu.Y = menu.RelY;
menu.W = argument2;
menu.H = argument3;
menu.options = argument4;
menu.textFont = argument5;
menu.textCol = argument6;
menu.fillCol = argument7;
menu.outlineCol = argument8;
menu.textAlpha = argument9;
menu.fillAlpha = argument10;
menu.outlineAlpha = argument11;
return menu;